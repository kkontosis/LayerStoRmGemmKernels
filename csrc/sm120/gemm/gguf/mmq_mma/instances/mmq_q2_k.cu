// Q2_K instance of the FAST GGUF mmq int8 tensor-core path (mmq_mma).
//
// Q2_K is a 16-subblock type: VALS=256, 16 sub-blocks of 16, min-type
// (B != 0). It uses the k=16 kernel body (gguf_mmq_mma_16subblock) — one MMA
// per 16-element sub-block — with the per-16-chunk affine rescale, which folds
// B*sumqx generically, so the nonzero B needs no special handling here. This is
// a near-copy of the Q6_K instance with only the policy and the launch wrapper
// name swapped. No kernel change needed.
...
static void launch_q2k_impl(const GgufMmvqParams& params,
                            layerstorm::formats::block_q8_1* xq,
                            cudaStream_t stream) {
    using namespace mmq_mma;
    using P = gguf_int::Q2K;
...
void gguf_mmq_mma_launch_q2k(const GgufMmvqParams& params,
    ...
    if (mmq_mma_use_small_m(params.M, params.N))
        launch_q2k_impl<true, SmallMTileCfg>(params, xq, stream);
    else
        launch_q2k_impl<true, DefaultTileCfg>(params, xq, stream);
}

// Benchmark/diagnostic hook: force the non-cp.async path.
void gguf_mmq_mma_launch_q2k_baseline(const GgufMmvqParams& params,
                                      layerstorm::formats::block_q8_1* xq,
                                      cudaStream_t stream) {
    launch_q2k_impl</*USE_CPASYNC=*/false>(params, xq, stream);
}

// Benchmark/diagnostic hook: force the small-M tile config (64x128).
void gguf_mmq_mma_launch_q2k_smallm(const GgufMmvqParams& params,
                                    layerstorm::formats::block_q8_1* xq,
                                    cudaStream_t stream) {
    launch_q2k_impl<true, mmq_mma::SmallMTileCfg>(params, xq, stream);
}

// ── Device-routed grouped mmq (TD-GG5) ─────────────────────────────────────
template <bool USE_CPASYNC, class Cfg>
static void launch_q2k_grouped_impl(const GgufGroupedMmqArgs& args, int T_max,
                                    cudaStream_t stream) {
    using namespace mmq_mma;
    using P = gguf_int::Q2K;
    constexpr int SMEM = SmemSizes16<P::VALS, Cfg::kTILE_M, Cfg::kTILE_N>::total;
...
void gguf_mmq_mma_grouped_launch_q2k(const GgufGroupedMmqArgs& args, bool use_small,
    ...
    using namespace mmq_mma;
    if (use_small) launch_q2k_grouped_impl<true, SmallMTileCfg>(args, T_max, stream);
    else           launch_q2k_grouped_impl<true, DefaultTileCfg>(args, T_max, stream);
}
